...
function mainControll(){
    ...
    view.distortionButton.onclick =()=>{
        ...
        if(model.distortionNode && model.panNode && model.audioCtx){
            if(model.distortionOn){
                ...
                model.panNode.connect(model.distortionNode).connect(model.audioCtx.destination);
            } 
        }
    }
    ...
    function init() {
        if(model.audioElement){
            ...
            model.track.connect(model.gainNode).connect(model.panNode).connect(model.audioCtx.destination);
        }
    }

    function setUpFaders() {
        view.volFader.addEventListener("input", () => {
            if(model.gainNode) model.gainNode.gain.value = parseFloat(view.volFader.value);
        }, false);
        view.panFader.addEventListener("input", () => {
            if(model.panNode) model.panNode.pan.value = parseFloat(view.panFader.value);
        }, false);
    }
